...
def test_early_phase():
    strategy = SeasonStrategy(make_parser(3))
    assert strategy.phase() == "EARLY"


def test_mid_phase():
    strategy = SeasonStrategy(make_parser(15))
    assert strategy.phase() == "MID"


def test_late_phase():
    strategy = SeasonStrategy(make_parser(24))
    assert strategy.phase() == "LATE"


def test_end_phase():
    strategy = SeasonStrategy(make_parser(29))
    assert strategy.phase() == "END"


def test_remaining_days():
    strategy = SeasonStrategy(make_parser(10))
    assert strategy.remaining_days() == 20


def test_can_plant_true():
    strategy = SeasonStrategy(make_parser(20))
...
def test_should_invest():
    strategy = SeasonStrategy(make_parser(5))
